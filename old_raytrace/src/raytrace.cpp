...
image4f raytrace(
    ...
    for(instance* inst : scn->instances){
        vec3f emiss = inst->mat->ke;
        if(emiss.x > 0 || emiss.y > 0 || emiss.z > 0){
            lights.push_back(inst);
        }
    }
    int squared_sample = samples*samples;
    int par = 8;
    int img_h_q = img.height / par;
    int img_w_q = img.width / par;    
    int img_h_r = img.height % par;
    int img_w_r = img.width % par;
    std::vector<std::future<void>> futures;
    printf("%d   %d\n", img_w_r, img_h_r);
    for(int t1 = 0; t1 < par; t1++){
        for(int t2 = 0; t2 < par; t2++){
            futures.push_back(std::async(std::launch::async, [&img_h_q, &img_w_q, t1, t2 ,&img, &samples, &cam, &lights, &amb, &scn, &squared_sample]{
                for(size_t i = 0  + (t1 * img_w_q); i < img_w_q * (t1 +1);  i++){
                    for(size_t j = 0 + (t2 * img_h_q); j < img_h_q * (t2 + 1); j++){
                        vec4f pixel = {};
                        for(size_t sample_i = 0 ; sample_i < samples; sample_i++){
                            for(size_t sample_j = 0; sample_j < samples; sample_j++){
                                float u = (i + (sample_i + 0.5f) /samples) /img.width;
                                float v = (img.height - j + (sample_j + 0.5f) /samples)/ img.height;
                                const vec2f uv = {u,v};
                                ray3f ray = eval_camera(cam, uv);
                                pixel += shade(scn, lights, amb, ray);
                            }
                        }
                        pixel.x /= squared_sample;
                        pixel.y /= squared_sample;
                        pixel.z /= squared_sample;
                        img.at(i,j) = pixel;
                    }
                }
            }));
