...
=======
// note1 : obj files, mtl files and png files for textures must be all in the
// same folder! note2 : every object in the scene, must be stored in the proper
// vector contained in the scene object(the pointer of it), in order to delete it
// when deleting the scene;
>>>>>>> 410c141bfb0ea5ec135e3378fc4d7110829ad1c9
...
void add_instance(scene* scn, const std::string& name, const frame3f& f, shape* shp) {
	if (!shp) return;
=======
// the type of element obtainable by subidiving facade. Only the terminal
// elements will be added to the scene and textured
enum element_type {
    facade,
    topfloors,
    topfloor,
...
// constants for residential buildings
static const building_constants house_info = {
    std::pair<float, float>(30.0, 100.0), std::pair<float, float>(4.0, 10.0),
    std::pair<float, float>(4.0, 10.0),
    std::pair<float, float>((5.0 / 6.0), (6.0 / 5.0)),
    std::pair<float, float>(2.5, 3.5), std::pair<float, float>(2.0, 2.5),
    std::pair<float, float>(2.0, 2.5), std::vector<std::string>(),
    std::vector<std::string>(), std::vector<std::string>(),
    std::vector<std::string>(), std::vector<std::string>(),
    std::map<element_type,
        std::pair<std::vector<element_type>, subdiv_axis>>()};

// constants for skyscrapers
static const building_constants skyscraper_info = {
    std::pair<float, float>(30.0, 100.0), std::pair<float, float>(4.0, 10.0),
    std::pair<float, float>(4.0, 10.0),
    std::pair<float, float>((5.0 / 6.0), (6.0 / 5.0)),
    std::pair<float, float>(2.5, 3.5), std::pair<float, float>(2.0, 2.5),
...
// add instance
void add_instance(
    scene* scn, const std::string& name, const frame3f& f, shape* shp) {
    if (!shp) return;
>>>>>>> 410c141bfb0ea5ec135e3378fc4d7110829ad1c9
    ...
    if (std::find(scn->instances.begin(), scn->instances.end(), inst) ==
        scn->instances.end()) {
        scn->instances.push_back(inst);
    }
    if (std::find(scn->shapes.begin(), scn->shapes.end(), shpgrp) ==
        scn->shapes.end()) {
...
void remove_shape_from_scene(scene* scn, shape* shp) {
    if (shp->mat != nullptr) {
        auto res1 =
            std::find(scn->materials.begin(), scn->materials.end(), shp->mat);
        if (res1 != scn->materials.end()) {
            scn->materials.erase(res1);
            delete shp->mat->kd_txt;
        }
        auto res2 = std::find(
            scn->textures.begin(), scn->textures.end(), shp->mat->kd_txt);
        if (res2 != scn->textures.end()) {
            scn->textures.erase(res2);
            delete shp->mat;
        }
...
void remove_shapes_from_scene(scene* scn, std::vector<shape*>& vecshp) {
    for (auto shp : vecshp) { remove_shape_from_scene(scn, shp); }
}
...
auto get_instance_by_shape(scene* scn, shape* shp) -> instance* {
    ...
    for (auto instance : scn->instances) {
        auto res = std::find(
            instance->shp->shapes.begin(), instance->shp->shapes.end(), shp);
        if (res != instance->shp->shapes.end()) {
            if (r != nullptr) std::cout << "Aiuto" << std::endl;
            r = instance;
>>>>>>> 410c141bfb0ea5ec135e3378fc4d7110829ad1c9
...
=======
std::vector<shape*> split_y(scene* scn, shape* shp, const std::vector<float> v,
    const std::vector<std::string> types) {
  //  std::cout << "Splitting y axis of shape " << shp->name << std::endl;
    float check = 0;
    for (int i = 0; i < v.size(); i++) { check += v.at(i); }
    std::vector<shape*> newv = std::vector<shape*>();
    if (check != 1.0f || v.size() != types.size()) {
        printf("unconsistent data %.10f\n", check); //
...
=======
std::vector<shape*> repeat_y(
    scene* scn, shape* shp, int parts, const std::string& type) {
    if (parts < 1) { return std::vector<shape*>(); }
    std::vector<float> v = std::vector<float>();
    std::vector<std::string> types = std::vector<std::string>();
>>>>>>> 410c141bfb0ea5ec135e3378fc4d7110829ad1c9
...
std::vector<shape*> split_x(scene* scn, shape* shp, std::vector<float> v, const std::vector<std::string> types) {
	...
	for (int j = 0; j < v.size(); j++) {
		...
		//std::cout << "Generating  shape " << nshp->name << std::endl;
=======
std::vector<shape*> split_x(scene* scn, shape* shp, std::vector<float> v,
    const std::vector<std::string> types) {
    //	std::cout << "Splitting x axis of shape " << shp->name << std::endl;
    float check = 0;
    for (int i = 0; i < v.size(); i++) { check += v.at(i); }
...
>>>>>>> 410c141bfb0ea5ec135e3378fc4d7110829ad1c9
    ...
    for (int j = 0; j < v.size(); j++) {
        ...
        x += v.at(j) * w;
        z += v.at(j) * l;
        //		std::cout << "Generating  shape " << nshp->name <<
        //std::endl;
    }
    return newv;
}
...
uint32_t calculate_floors(uint32_t height) {
    uint32_t floor_h = height / 3.5;  // hardocded for now TODO, make chose from
                                    // range, maybe using an apporximation
    std::cout << "Height: " << height << "   Number of foors: " << floor_h
              << std::endl;
    return floor_h;
}
...
std::vector<shape*> subdiv_facade(scene* scn, instance* inst, shape* shp) {
    ...
    if (shp->name == "facade") {
        for (shape* nshp : split_y(scn, shp, std::vector<float>{0.1, 0.9},
                 std::vector<std::string>{"bottomfloor", "topfloors"})) {
            auto res = subdiv_facade(scn, inst, nshp);
            std::copy(res.begin(), res.end(), back_inserter(to_add));
            delete nshp;
        }
    } else if (shp->name == "topfloors") {
        for (shape* nshp : repeat_y(scn, shp, calculate_floors(h), "floor")) {
            auto res = subdiv_facade(scn, inst, nshp);
            std::copy(res.begin(), res.end(), back_inserter(to_add));
            delete nshp;
        }
    } else if (shp->name == "bottomfloor") {
        for (shape* nshp : split_x(scn, shp, std::vector<float>{0.4, 0.2, 0.4},
                 std::vector<std::string>{"tiles", "doortile", "tiles"})) {
            auto res = subdiv_facade(scn, inst, nshp);
            std::copy(res.begin(), res.end(), back_inserter(to_add));
            delete nshp;
        }
    } else if (shp->name == "doortile") {
        for (shape* nshp : split_x(scn, shp, std::vector<float>{0.1, 0.8, 0.1},
                 std::vector<std::string>{"vwall", "doorcol", "vwall"})) {
            auto res = subdiv_facade(scn, inst, nshp);
            std::copy(res.begin(), res.end(), back_inserter(to_add));
            if (nshp->name == "doorcol") delete nshp;
        }
    } else if (shp->name == "doorcol") {
        for (shape* nshp : split_y(scn, shp, std::vector<float>{0.9, 0.1},
                 std::vector<std::string>{"door", "hwall"})) {
            auto res = subdiv_facade(scn, inst, nshp);
            std::copy(res.begin(), res.end(), back_inserter(to_add));
        }
        ...
        for (shape* nshp : split_x(scn, shp, std::vector<float>{0.1, 0.8, 0.1},
                 std::vector<std::string>{"vwall", "windcol", "vwall"})) {
            auto res = subdiv_facade(scn, inst, nshp);
            std::copy(res.begin(), res.end(), back_inserter(to_add));
            if (nshp->name == "windcol") delete nshp;
        }
    } else if (shp->name == "windcol") {
        for (shape* nshp : split_y(scn, shp, std::vector<float>{0.1, 0.8, 0.1},
                 std::vector<std::string>{"hwall", "window", "hwall"})) {
            auto res = subdiv_facade(scn, inst, nshp);
            std::copy(res.begin(), res.end(), back_inserter(to_add));
        }
    } else {
...
=======
    for (shape* shp : inst->shp->shapes) {
        if (shp->name == "roof") {
            shp->mat =
                make_material("roof", vec3f{1.0f, 1.0f, 1.0f}, "roof.png");
            // shp->subdivision = 5;
            // // tesselate_shape(shp, true, false, false, false);
            // // shp->mat->disp_txt = new texture{ "roof", "displace_roof_test"
            // };	//test
        }
        if (shp->name == "vwall") {
            shp->mat =
                make_material("vwall", vec3f{1.0f, 1.0f, 1.0f}, "vwall.png");
        }
        if (shp->name == "hwall") {
            shp->mat =
                make_material("hwall", vec3f{1.0f, 1.0f, 1.0f}, "hwall.png");
        } else if (shp->name == "window") {
            shp->mat = make_material("window", vec3f{1.0f, 1.0f, 1.0f},
                "window.png", vec3f{0.4f, 0.4f, 0.4f});
        } else if (shp->name == "door") {
            shp->mat =
                make_material("door", vec3f{1.0f, 1.0f, 1.0f}, "door.png");
        }
...
int main(int argc, char** argv) {
    // parsing
    auto parser = make_parser(argc, argv, "model", "creates simple scenes");
    auto sceneout =
        parse_opt(parser, "--output", "-o", "output scene", "out.obj"s);

    // printf("creating scene %s\n", type.c_str());

    // make dummy material
    // material* mat = make_material("building", { 1.0f, 1.0f, 1.0f },
    // "colored.png");
    // make base
  ...
  //  make_map(scn);
    ...
    add_instance(scn, "base",
        frame3f{{1, 0, 0}, {0, 1, 0}, {0, 0, 1}, {0, 1.25f, 0}}, base);
    // add_instance(scn, "base", frame3f{ { 1, 0, 0 },{ 0, 1, 0 },{ 0, 0, 1 },{
    // 0, 1.25f, 0 } }, base2);
    ...
    std::uniform_int_distribution<uint32_t> h_range(
        skyscraper_info.h_range.first, skyscraper_info.h_range.second);
    auto hight = bind_random_distribution(h_range);
    h = hight();
    extrude(scn, building, h);
    ...
    for (instance* inst : scn->instances) {
        ...
        for (auto shp : to_add) { inst->shp->shapes.push_back(shp); }
    }
...
=======
    //
    for (instance* inst : scn->instances) {
        if (inst->name == "building") { apply_material_and_texture(scn, inst); }
    }
>>>>>>> 410c141bfb0ea5ec135e3378fc4d7110829ad1c9
